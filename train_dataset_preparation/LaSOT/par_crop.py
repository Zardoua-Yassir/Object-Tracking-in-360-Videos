...
def crop_video(video, crop_path, instanc_size, num):
    ...
    for idx, gt_line in enumerate(groundtruth):
        ...
        # z_path = join(video_crop_base_path, '{:06d}.{:02d}.z.jpg'.format(int(idx), int(0)))
        x_path = join(video_crop_base_path, '{:06d}.{:02d}.x.jpg'.format(int(idx), int(0)))
        print("x path:\n{}".format(x_path))
        # print("z path:\n{}\n x path:\n{}\n\n".format(z_path, x_path))
        # cv2.imwrite(z_path, z)
        cv2.imwrite(x_path, x)


def main(instanc_size=511, num_threads=4):
    # crop_path = './window{:d}'.format(instanc_size)
    crop_path = r"D:\LaSOTPrepared\crop{:d}".format(instanc_size)
